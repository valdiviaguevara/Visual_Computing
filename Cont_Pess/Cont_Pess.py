...
def center(x, y, w, h):
    ...
    cy = y + y1
    return cx,cy
#Capturando o videio do mp4
cap = cv2.VideoCapture('1.mp4')
#Abrindo a metodologia que elimina os pixeles errados da sombra
...
while 1:
    #Capturando cada frame do Videio
    ret, frame = cap.read()
    #Convertendo o videio em gris
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    #cv2.imshow("gray", gray)
    #eliminando as imagenes estaticas comum so cor (preto)
    fgmask = fgbg.apply(gray)
    #cv2.imshow("fgmask", fgmask)
    #limpando alguns defeitos de pixeles da originacao da sombra
    retval, th = cv2.threshold(fgmask, 200, 255, cv2.THRESH_BINARY)
    #cv2.imshow("th", th)
    #Convertendo o conjunto de pixeles moviveis em forma de Elipse
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
    opening = cv2.morphologyEx(th, cv2.MORPH_OPEN, kernel, iterations = 2)
    #cv2.imshow("opening", opening)
    #dilatando os objetos em movimento para concentrar em um só
    ...
    #limitando com uma linha meu escenario de medida
    cv2.line(frame,xy1,xy2,(255,0,0),3)
    cv2.line(frame,(xy1[0],posL-offset),(xy2[0],posL-offset),(255,255,0),2)
    cv2.line(frame,(xy1[0],posL+offset),(xy2[0],posL+offset),(255,255,0),2)
    contours, hierarchy = cv2.findContours(dilation,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
    i = 0
    #Fazendo a contagem de Pessoas pasando pela linha limitada do videio
    for cnt in contours:
